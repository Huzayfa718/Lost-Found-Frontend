  ...
  useEffect(() => {
    fetch('http://localhost:5000', {
      ...
      .then((data) => {
        ...
        const lostItems = list.filter(
          (item) => item.postType === 'Lost' && item.status === 'unrecovered'
        );
        setItems(lostItems);
        setLoading(false);
      })
    ...
    <div className="bg-slate-50 min-h-screen">
      <section className="bg-slate-950 text-white">
        <div className="max-w-7xl mx-auto px-4 py-10">
          <p className="text-sm uppercase tracking-widest text-blue-200 font-semibold">Missing reports</p>
          <h2 className="text-3xl md:text-4xl font-extrabold mt-2">Lost Items</h2>
          <p className="text-slate-300 mt-3 max-w-2xl">
            Search reported lost belongings by title, location, category, or description.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm mb-8">
          <div className="grid lg:grid-cols-[1fr_220px_180px] gap-4">
            <label className="form-control">
              <span className="label-text mb-2 font-semibold text-slate-800">Search lost items</span>
              <input
                value={search}
                onChange={(event) => setSearch(event.target.value)}
                type="search"
                placeholder="Try wallet, phone, campus, ID card..."
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm font-medium text-slate-600">
              Showing {filteredItems.length} of {items.length} lost reports
            </p>
            {(search || category !== 'All' || sortBy !== 'Newest') && (
              <button
                ...
                className="btn btn-sm btn-outline border-slate-300 text-slate-700 hover:bg-blue-50 hover:border-blue-400 hover:text-blue-700"
              >
                Clear Filters
              </button>
            )}
        ...
        {filteredItems.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-lg p-10 text-center shadow-sm">
            <h3 className="text-xl font-bold text-slate-950">No lost items matched</h3>
            <p className="text-slate-500 mt-2">Try another keyword or category.</p>
            <Link to="/addItems" className="btn btn-primary mt-5">
              Report Lost Item
            </Link>
          </div>
        ) : (
